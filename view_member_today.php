...
if ($_SESSION['user_level'] <2  || $_SESSION['user_level'] >2 ) {	
	$url = BASE_URL . 'view_state_member_today.php'; // Define the URL.
	ob_end_clean(); // Delete the buffer.
	header("Location: $url");
	exit(); // Quit the script.	
}
...
$q = "SELECT last_name, first_name, phone, user_level,  DATE_FORMAT(created_at, '%M %d, %Y') AS dr, user_id FROM users WHERE created_at >= DATE_SUB(NOW(), INTERVAL 24 HOUR) ";		
...
// Define the query:
$q = "SELECT last_name, first_name, phone, user_level, DATE_FORMAT(created_at, '%M %d, %Y') AS dr, user_id FROM users WHERE created_at >= DATE_SUB(NOW(), INTERVAL 24 HOUR)  ORDER BY $order_by LIMIT $start, $display";		
$r = @mysqli_query ($dbc, $q); // Run the query.
if (!$r) {
    printf("Error: %s\n", mysqli_error($dbc));
    exit();
...
// Table header:
echo "<h3 style='color:#036622;'><strong align='center'>There are currently $num Exco Members</strong></h3>\n <hr>";
...
<tr>
	...
	<th align="left"><b><a href="view_users.php?sort=ul">User Role</a></b></th>
