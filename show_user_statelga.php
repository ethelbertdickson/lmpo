...
if ($_SESSION['user_level'] <2  || $_SESSION['user_level'] >16 ) {	
	$url = BASE_URL . 'show_user_statelga.php'; // Define the URL.
	ob_end_clean(); // Delete the buffer.
	header("Location: $url");
	exit(); // Quit the script.	
}
...
// Check for a valid user ID, through GET or POST:
    if ( (isset($_GET['lid'])) && (is_numeric($_GET['lid'])) ) { // From show_user_by_lga.php
        $id = $_GET['lid'];
    } elseif ( (isset($_POST['lid'])) && (is_numeric($_POST['lid'])) ) { // Form submission.
        $id = $_POST['lid'];
...
if (isset($_GET['p']) && is_numeric($_GET['p'])) { // Already been determined.
	...
	$q = "SELECT COUNT(lga_id) FROM users WHERE lga_id = $id AND state_id = $state_code";
...
$q = "SELECT last_name, first_name, phone, user_level,  DATE_FORMAT(created_at, '%M %d, %Y') AS dr, user_id FROM users WHERE lga_id = $id AND state_id = $state_code";		
$r = @mysqli_query ($dbc, $q); // Run the query.
// Count the number of returned rows:
$num = mysqli_num_rows($r);

if ($num > 0) { // If it ran OK, display the records.
    $q = "SELECT lga_name FROM  lga WHERE lga_id = $id";
    $r = @mysqli_query ($dbc, $q); // Run the query.
...
// Define the query:
$q = "SELECT last_name, first_name, phone, user_level, DATE_FORMAT(created_at, '%M %d, %Y') AS dr, user_id FROM users where lga_id = $id AND state_id = $state_code ORDER BY $order_by LIMIT $start, $display";		
$r = @mysqli_query ($dbc, $q); // Run the query.
if (!$r) {
    printf("Error: %s\n", mysqli_error($dbc));
    exit();
...
// Table header:
echo "<h3 style='color:#036622;'><strong align='center'>There are currently $num registered Members in $lga_name LGA</strong></h3>\n <hr>";
...
if ($pages > 1) {
	...
	if ($current_page != 1) {
		echo '<a href="show_user_by_lga.php?id='.$id.'&s=' . ($start - $display) . '&p=' . $pages . '&sort=' . $sort . '"><input name="button" type="button" value="Previous" class="btn btn-primary"/></a> ';
	}
	
	// Make all the numbered pages:
	for ($i = 1; $i <= $pages; $i++) {
		if ($i != $current_page) {
			echo '<a href="show_user_by_lga.php?id='.$id.'&s=' . (($display * ($i - 1))) . '&p=' . $pages . '&sort=' . $sort . '">' . $i . '</a> ';
		} else {
			echo $i . ' ';
		}
	} // End of FOR loop.
	...
	if ($current_page != $pages) {
		echo '<a href="show_user_by_lga.php?id='.$id.'&s=' . ($start + $display) . '&p=' . $pages . '&sort=' . $sort . '"><input name="button" type="button" value="Next" class="btn btn-primary"/></a>';
	}
