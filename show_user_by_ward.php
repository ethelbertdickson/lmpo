...
if (isset($_GET['p']) && is_numeric($_GET['p'])) { // Already been determined.
  ...
  $q = 'SELECT COUNT(ward_id) FROM users WHERE ward_id = "$wid"';
...
$q = "SELECT last_name, first_name, phone, user_level,  DATE_FORMAT(created_at, '%M %d, %Y') AS dr, user_id FROM users WHERE ward_id LIKE '$wid%'";   
$r = @mysqli_query ($dbc, $q); // Run the query.
// Count the number of returned rows:
$num = mysqli_num_rows($r);

if ($num > 0) { // If it ran OK, display the records.
    $q = "SELECT ward_id, ward_name FROM  ward WHERE ward_id LIKE '$wid%' ";
    $r = @mysqli_query ($dbc, $q); // Run the query.
...
// Define the query:
$q = "SELECT last_name, first_name, phone, user_level, DATE_FORMAT(created_at, '%M %d, %Y') AS dr, user_id FROM users where ward_id LIKE '$wid%' ORDER BY $order_by LIMIT $start, $display";    
$r = @mysqli_query ($dbc, $q); // Run the query.
if (!$r) {
    printf("Error: %s\n", mysqli_error($dbc));
    exit();
