...
if (isset($_GET['p']) && is_numeric($_GET['p'])) { // Already been determined.
  ...
  $q = "SELECT COUNT(state_id) FROM users WHERE state_id = $id";
...
$q = "SELECT last_name, first_name, phone, user_level,  DATE_FORMAT(created_at, '%M %d, %Y') AS dr, user_id FROM users WHERE state_id = $id";   
...
// Define the query:
$q = "SELECT last_name, first_name, phone, user_level, DATE_FORMAT(created_at, '%M %d, %Y') AS dr, user_id FROM users where state_id = $id ORDER BY $order_by LIMIT $start, $display";    
$r = @mysqli_query ($dbc, $q); // Run the query.
if (!$r) {
    printf("Error: %s\n", mysqli_error($dbc));
    exit();
...
if ($pages > 1) {
  ...
  if ($current_page != 1) {
    echo '<a href="show_user_by_state.php?id='.$id.'&s=' . ($start - $display) . '&p=' . $pages . '&sort=' . $sort . '"><input name="button" type="button" value="Previous" class="btn btn-primary"/></a> ';
  }
  
  // Make all the numbered pages:
  for ($i = 1; $i <= $pages; $i++) {
    if ($i != $current_page) {
      echo '<a href="show_user_by_state.php?id='.$id.'&s=' . (($display * ($i - 1))) . '&p=' . $pages . '&sort=' . $sort . '">' . $i . '</a> ';
    } else {
      echo $i . ' ';
    }
  } // End of FOR loop.
  ...
  if ($current_page != $pages) {
    echo '<a href="show_user_by_state.php?id='.$id.'&s=' . ($start + $display) . '&p=' . $pages . '&sort=' . $sort . '"><input name="button" type="button" value="Next" class="btn btn-primary"/></a>';
  }
