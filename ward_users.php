...
$page_title = ' Registered Members by ward';
...
if (isset($_GET['p']) && is_numeric($_GET['p'])) { // Already been determined.
  ...
  $q = "SELECT COUNT(ward_id) FROM users";
...
// Define the query:
$q = 
"SELECT state_name, lga_name, ward_name, u.ward_id AS wid, u.state_id AS stid, u.lga_id AS lid, count(u.first_name) AS user_count 
FROM ward AS wa 
JOIN users AS u   ON  u.ward_id = wa.ward_id
JOIN lga AS lg    ON  u.lga_id = lg.lga_id
JOIN states AS st ON  u.state_id = st.state_id 
GROUP BY wa.ward_id ORDER BY state_name, lga_name, user_count DESC  LIMIT $start, $display ";
...
while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
    ...
    <td align="left"><a href="show_user_by_ward.php?wid=' . $row['wid'] . '&stid=' . $row['stid'] . '&lid=' . $row['lid'] . '"><span class="btn btn-danger"><i class="fas fa-eye"></i> View Members</span></a></td>
...
if ($pages > 1) {
  ...
  if ($current_page != 1) {
    echo '<a href="ward_users.php?s=' . ($start - $display) . '&p=' . $pages . '"><input name="button" type="button" value="Previous" class="btn btn-primary"/></a> ';
  }
  
  // Make all the numbered pages:
  for ($i = 1; $i <= $pages; $i++) {
    if ($i != $current_page) {
      echo '<a href="ward_users.php?s=' . (($display * ($i - 1))) . '&p=' . $pages . '">' . $i . '</a> ';
    } else {
      echo $i . ' ';
    }
  } // End of FOR loop.
  ...
  if ($current_page != $pages) {
    echo '<a href="ward_users.php?s=' . ($start + $display) . '&p=' . $pages  . '"><input name="button" type="button" value="Next" class="btn btn-primary"/></a>';
  }
