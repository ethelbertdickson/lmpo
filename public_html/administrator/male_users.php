...
$q = "SELECT last_name, first_name, phone, user_level,  DATE_FORMAT(created_at, '%M %d, %Y') AS dr, user_id FROM users WHERE gender ='1' ";		
...
// Define the query:
$q = "SELECT last_name, first_name, phone, user_level, DATE_FORMAT(created_at, '%M %d, %Y') AS dr, user_id FROM users WHERE gender ='1'  ORDER BY $order_by LIMIT $start, $display";		
$r = @mysqli_query ($dbc, $q); // Run the query.
if (!$r) {
    printf("Error: %s\n", mysqli_error($dbc));
    exit();
...
// Table header:
echo "<h3 style='color:#036622;'><strong align='center'>There are currently $num Male Members</strong></h3>\n <hr>";
